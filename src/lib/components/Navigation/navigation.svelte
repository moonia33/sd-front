	...
	<div class="relative md:flex md:items-center md:justify-between">
		<div class="flex items-center justify-between">
			...
			<div class="d-flex flex md:hidden">
				...
				<button
					type="button"
					class="hs-collapse-toggle border-layer-line text-layer-foreground hover:bg-muted-hover focus:bg-muted-focus ml-2 flex size-9 items-center justify-center rounded-lg border text-sm font-semibold focus:outline-hidden disabled:pointer-events-none disabled:opacity-50"
				...
				>
					{#each menuItems ?? [] as item (item.id)}
						{#if item.children?.length}
							...
							<div
								class="hs-dropdown py-3 [--adaptive:none] [--is-collapse:true] [--strategy:static] md:py-4 md:[--is-collapse:false] md:[--strategy:absolute] md:[--trigger:hover]"
							>
								<button
									type="button"
									class="text-navbar-nav-foreground hover:text-muted-foreground-1 focus:text-muted-foreground-1 flex w-full items-center font-medium focus:outline-hidden"
									aria-haspopup="menu"
									aria-expanded="false"
								>
									{item.title}
												...
												<div class="flex flex-col">
													{#each leftColumn as child (child.id)}
														{#if child.href}
															...
															<a
																class="group text-navbar-nav-foreground hover:bg-navbar-nav-hover focus:bg-navbar-nav-focus flex gap-x-4 rounded-lg p-3 focus:outline-hidden"
